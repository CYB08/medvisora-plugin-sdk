...
- `label`：该结构映射到的平台标签，两个可选号段见下表
- `semantic`：结构名称，插件自定义区必填，系统区不得声明
...
| 号段 | 由谁命名 | `semantic` |
| --- | --- | --- |
| `1–499` | 平台。只能用系统标签表中已分配的 ID。 | 不得声明 |
| `500–999` | 你自己。用于系统标签表尚未覆盖的结构。 | 必填 |

结构在系统表里有对应 ID 时优先用系统区：这些 ID 有稳定的名称、固定的颜色和跨插件一致的含义，不同模型卡的结果因此可以直接对比。`500` 以下尚未分配的 ID 是平台自己的扩号位，会被拒绝——插件占了之后平台再分配就会撞车，而存盘的 mask 里记的只是这个数值。

系统表里没有的结构用 `500–999`。名称取自你声明的 `semantic`，在你的模型卡处于安装状态期间显示；卸载后该结构回落为占位名。这一段没有全局仲裁者，因此注册时会拒绝已被其他已装模型卡用于别的结构的 ID——换一个未被占用的号即可。

```json
{
  "outputs": [
    { "model": 1, "label": 5 },
    { "model": 2, "label": 500, "semantic": "支气管" }
  ]
}
```

未声明 `outputs` 时，平台保留 mask 的原始值。实例分割节点不得声明 `outputs`。
